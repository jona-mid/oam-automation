...
- **Retry:** failed uploads are logged per candidate and never appended to the ledger, but auto-derived windows do not re-cover them. The next run's window starts at the last scrape's newest upload, which is at or after every failed candidate. To retry, re-run with an explicit `--uploaded-after` covering the failed run's window (the wrapper prints the exact command when uploads fail); the dedup legs skip everything already uploaded.
