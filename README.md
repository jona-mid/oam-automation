...
- `--uploaded-before YYYY-MM-DD` bounds ad-hoc windows from above: the run only processes OAM uploads between `--uploaded-after` and this date, including the whole before day (an upload on the before date at any time of day is in). The weekly default never sets it. The flag requires a resolvable `--uploaded-after` with a common period in between, otherwise the run aborts. Use a fresh `--output-dir` for each ad-hoc window; the pipeline skips stages whose outputs already exist, and a reused dir that holds a different window is rejected before any pipeline work (dry runs too, when window flags are passed; a plain dry run without flags just inspects whatever the dir holds).
- Ad-hoc runs share the weekly status file (`last_run.txt`): the weekly chain value (`scrape_uploaded_at`) is only advanced by a run whose window actually covers the chain point, so ad-hoc windows starting ahead of the chain warn about the uncovered gap instead of silently skipping it, and dry runs never advance the chain at all.
...
- **Retry:** failed uploads are logged per candidate and never appended to the ledger, but auto-derived windows do not re-cover them. The next run's window starts at the last scrape's newest upload, which is at or after every failed candidate. To retry, re-run with an explicit `--uploaded-after` covering the failed run's window (the wrapper prints the exact command, including `--uploaded-before` when the failed run had one, whenever a run fails after scraping); the dedup legs skip everything already uploaded.
